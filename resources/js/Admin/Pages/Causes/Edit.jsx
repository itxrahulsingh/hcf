    ...
    const [faqs, setFaqs] = useState(() => {
        ...
        Object.keys(languages).forEach((code) => {
            try {
                ...
                result[code] = Array.isArray(parsed) && parsed.length > 0
                    ? parsed
                    : [{ title: "", content: "" }]
            } catch {
                        ...
                        <div className="yoo-card yoo-style1">
                            ...
                            <div className="yoo-card-body">
                                <div className="yoo-padd-lr-20">
                                    ...
                                    {/* Content & Projects */}
                                    <div className="row mt-3"><div className="col-md-12"><label>{translate("Content")}</label><Editor onChange={(v) => setData(`${selectedLang}_content`, v)} value={data[`${selectedLang}_content`]} /></div></div>
                                    <div className="row mt-3"><div className="col-md-12"><label>{translate("Projects")}</label><Editor onChange={(v) => setData(`${selectedLang}_projects`, v)} value={data[`${selectedLang}_projects`]} /></div></div>
                                                ...
                                                <div key={index} className="p-3 mb-3 border rounded">
                                                    ...
                                                    <div className="text-right">
                                                        {(faqs[selectedLang] || []).length > 1 && (
                                                            <button type="button" className="btn btn-danger btn-sm" onClick={() => removeFaq(index)}>{translate("Remove")}</button>
                                                        )}
                                                    </div>
                                                </div>
                                            ))}
                                    ...
                                    <div className="row mt-3"><div className="col-md-12"><label>{translate("Updates")}</label><Editor onChange={(v) => setData(`${selectedLang}_updates`, v)} value={data[`${selectedLang}_updates`]} /></div></div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div className="yoo-card-body">
                                <div className="yoo-padd-lr-20">
                                    ...
                                    <MultipleMediaUploader
                                        onSelected={(e) => setData(produce(d => {d.gallery_images = e}))}
                                        handleRemoved={(d) => setData(produce(draft => { draft.gallery_images = d }))}
                                        defaultValue={data.gallery_images}
                                    />
                                    <FormValidationError message={errors?.gallery_images} />

                                    <div className="mt-4"><button type="submit" disabled={processing} className="btn btn-success">{translate("Update")}</button></div>
                                </div>
                            </div>
                        </div>
                    </div>

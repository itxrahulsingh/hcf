...
export default function CaseStudyDetails2({ data }) {
    ...
    return (
        <div className="container">
            {banner_type === "static" && (
                <>{image_url && <img src={image_url} alt="Casestudy" className="cs_radius_50_0_0_0" style={{ marginBottom: "30px" }} loading="lazy" decoding="async"/>}</>
            )}
            {banner_type === "slider" && (
                    ...
                    <Swiper navigation={true} modules={[Navigation]} className="mySwiper" loop={true} speed={800}>
                        {gallery_list?.map((galleryItem, index) => (
                            <SwiperSlide key={index}>
                                <img src={galleryItem.gallery_image_url} alt="Casestudy" loading="lazy" decoding="async"/>
                            </SwiperSlide>
                        ))}
                    </Swiper>
                </div>
